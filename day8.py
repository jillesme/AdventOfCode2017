...
regex = r'^([a-z]+)\s+(inc|dec)\s+([0-9-]+)\sif\s([a-z]+)\s([<>!=]{1,2})\s([0-9]+)'
...
for match in matches:
    groups = match.groups()
    instructions.append({ 'register': groups[0],
            'action': groups[1],
            'value': int(groups[2]),
            'register_check': groups[3],
            'condition': groups[4],
            'value_check': int(groups[5]) })
...
for instruction in instructions:
    if instruction['condition'] == '<':
        execute = memory[instruction['register_check']] < instruction['value_check']
    if instruction['condition'] == '<=':
        execute = memory[instruction['register_check']] <= instruction['value_check']
    if instruction['condition'] == '>':
        execute = memory[instruction['register_check']] > instruction['value_check']
    ...
    if execute:
        if instruction['action'] == 'inc':
            memory[instruction['register']] += instruction['value']

        elif instruction['action'] == 'dec':
            memory[instruction['register']] -= instruction['value']

    values.append(memory[instruction['register']])

print(max(memory.values()), max(values))
